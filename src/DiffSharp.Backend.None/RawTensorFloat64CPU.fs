...
type RawTensorFloat64CPU(values: double[], shape:int[]) =
    ...
    override t1.Conv1D(t2, stride, padding) = upcast (RawTensorCPU.Conv1D(t1, t2, stride, padding) : RawTensorFloat64CPU)
    ...
    override t.SqueezeT(dim) = upcast (RawTensorCPU.SqueezeT(t, dim) : RawTensorFloat64CPU)
    override t.UnsqueezeT(dim) = upcast (RawTensorCPU.UnsqueezeT(t, dim) : RawTensorFloat64CPU)
    override t.ViewT(shape:int[]) = upcast (RawTensorCPU.ViewT(t, shape) : RawTensorFloat64CPU)
