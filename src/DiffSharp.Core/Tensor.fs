...
type Tensor = 
    ...
    static member Unstack (a:Tensor, ?dim:int) =
        ...
        | TensorR(ap,_,_,_,at) -> Array.mapi (fun i p -> TensorR(p, ref (p.Zero()), UnstackT(a, dim, i), ref 0u, at)) (ap.Unstack(dim))
    ...
    static member Split (a:Tensor, sizes: seq<int>, ?dim: int) =
        ...
        | TensorR(ap,_,_,_,at) -> Array.mapi (fun i p -> TensorR(p, ref (p.Zero()), SplitT(a, sizes, dim, i), ref 0u, at)) (ap.Split(sizes, dim=dim))
                ...
                | TensorR(_,_,o,_,_) ->
                    ...
                    if t.Fanout = 0u then
                        ...
                        | SplitT(a,sizes,dim,i) -> 
                            ...
                            a.Derivative <- Tensor.AddSlice(a.Derivative, Array.init a.Dim (fun j -> if j=dim then locs.[i] else 0), t.Derivative)
                            push ((a.Zero(), a) :: tt)
