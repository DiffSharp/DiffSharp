...
type DiffSharp =
    ...
    static member multinomial(probs:Tensor, numSamples:int, ?dtype:DType, ?device:Device, ?backend:Backend) = Tensor.multinomial(probs, numSamples, ?dtype=dtype, ?device=device, ?backend=backend)
    static member multinomial(numSamples:int, ?dtype:DType, ?device:Device, ?backend:Backend) = fun (probs:Tensor) -> Tensor.multinomial(probs, numSamples, ?dtype=dtype, ?device=device, ?backend=backend)
    static member bernoulli(probs:Tensor, ?dtype:DType, ?device:Device, ?backend:Backend) = Tensor.bernoulli(probs, ?dtype=dtype, ?device=device, ?backend=backend)
    static member bernoulli(?dtype:DType, ?device:Device, ?backend:Backend) = fun (probs:Tensor) -> Tensor.bernoulli(probs, ?dtype=dtype, ?device=device, ?backend=backend)
