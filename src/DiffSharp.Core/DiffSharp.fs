...
module DiffSharp =
    ...
    let inline makeForward (tag:uint32) (derivative:Tensor) (tensor:Tensor) = tensor.ForwardDiff(derivative, tag)
    let inline makeReverse (tag:uint32) (tensor:Tensor) = tensor.ReverseDiff(tag)
    let inline reverseReset (tensor:Tensor) = tensor.ReverseReset(true)
