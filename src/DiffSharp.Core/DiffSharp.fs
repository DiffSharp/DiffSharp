...
type DiffSharp =
    ...
    static member randint(max: int64, shape:seq<int>, ?dtype:DType, ?device:Device, ?backend:Backend) = Tensor(RawTensor.RandomIntegers(max, shape|>Seq.toArray, ?dtype=dtype, ?device=device, ?backend=backend))
    static member randint(max: int64, length:int, ?dtype:DType, ?device:Device, ?backend:Backend) = Tensor(RawTensor.RandomIntegers(max, [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
