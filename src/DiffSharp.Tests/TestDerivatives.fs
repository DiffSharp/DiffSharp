...
type TestDerivatives () =
    ...
    member this.TestDerivativeAddTT () =
        let fwdx = Tensor.Create([1.; 2.; 3.]).GetForward(Tensor.Create([2.; 3.; 4.]))
        let fwdy = Tensor.Create([5.; 6.; 7.]).GetForward(Tensor.Create([2.; 2.; 3.]))
        let fwdz = fwdx + fwdy
        let fwdzCorrect = Tensor.Create([6.; 8.; 10.])
        let fwdzd = fwdz.Derivative
        let fwdzdCorrect = Tensor.Create([4.; 5.; 7.])

        let revx = Tensor.Create([1.; 2.; 3.]).GetReverse()
        let revy = Tensor.Create([5.; 6.; 7.]).GetReverse()
        let revz = revx + revy
    ...
    member this.TestDerivativeAddT2T1 () =
        let fwdx = Tensor.Create([[1.; 2.]; [3.; 4.]]).GetForward(Tensor.Create([[2.; 3.]; [4.; 5.]]))
        let fwdy = Tensor.Create([5.; 6.]).GetForward(Tensor.Create([2.; 3.]))
        let fwdz = fwdx + fwdy
        let fwdzCorrect = Tensor.Create([[6.; 8.]; [8.; 10.]])
        let fwdzd = fwdz.Derivative
        let fwdzdCorrect = Tensor.Create([[4.; 6.]; [6.; 8.]])

        let revx = Tensor.Create([[1.; 2.]; [3.; 4.]]).GetReverse()
        let revy = Tensor.Create([5.; 6.]).GetReverse()
        let revz = revx + revy
    ...
    member this.TestDerivativeMatMulT2T2 () =
        ...
        let revx = Tensor.Create([[6.2381; 0.0393; 8.2364; 3.9906; 6.2291];
            [9.8762; 3.2263; 6.2866; 4.7111; 0.0652];
            [3.5832; 7.9801; 1.9854; 4.4965; 4.1712]]).GetReverse()
        let revy = Tensor.Create([[4.4220; 3.7293];
            [6.1928; 2.1446];
            [0.0525; 1.2494];
            [7.5281; 1.4816];
            [5.0328; 2.2756]]).GetReverse()
    ...
    member this.TestTensorStackTs () =
        let fwdxa = Tensor.Create([1.; 2.]).GetForward(Tensor.Create([10.; 20.]))
        let fwdxb = Tensor.Create([3.; 4.]).GetForward(Tensor.Create([30.; 40.]))
        let fwdxc = Tensor.Create([5.; 6.]).GetForward(Tensor.Create([50.; 60.]))
        ...
        let revxa = Tensor.Create([1.; 2.]).GetReverse()
        let revxb = Tensor.Create([3.; 4.]).GetReverse()
        let revxc = Tensor.Create([5.; 6.]).GetReverse()
    ...
    member this.TestDerivativeTransposeT2 () =
        let fwdx = Tensor.Create([[1.; 2.; 3.]; [4.; 5.; 6.]]).GetForward(Tensor.Create([[2.; 3.; 4.]; [10.; 20.; 30.]]))
        let fwdz = fwdx.Transpose()
        let fwdzCorrect = Tensor.Create([[1.; 4.]; [2.; 5.]; [3.; 6.]])
        let fwdzd = fwdz.Derivative
        let fwdzdCorrect = Tensor.Create([[2.; 10.]; [3.; 20.]; [4.; 30.]])

        let revx = Tensor.Create([[1.; 2.; 3.]; [4.; 5.; 6.]]).GetReverse()
    ...
    member this.TestDerivativeSignT () =
        let fwdx = Tensor.Create([-1.; 0.; 3.]).GetForward(Tensor.Create([2.; 3.; 4.]))
        let fwdz = fwdx.Sign()
        let fwdzCorrect = Tensor.Create([-1.; 0.; 1.])
        let fwdzd = fwdz.Derivative
        let fwdzdCorrect = Tensor.Create([0.; 0.; 0.])

        let revx = Tensor.Create([-1.; 0.; 3.]).GetReverse()
        let revz = revx.Sign()
    ...
    member this.TestDerivativeReluT () =
        let fwdx = Tensor.Create([-1.; -2.; 0.; 3.; 10.]).GetForward(Tensor.Create([2.; 3.; 4.; 5.; 6.]))
        let fwdz = fwdx.Relu()
        let fwdzCorrect = Tensor.Create([0.; 0.; 0.; 3.; 10.])
        let fwdzd = fwdz.Derivative
        let fwdzdCorrect = Tensor.Create([0.; 0.; 0.; 5.; 6.])

        let revx = Tensor.Create([-1.; -2.; 0.; 3.; 10.]).GetReverse()
        let revz = revx.Relu()
    ...
    member this.TestDerivativeExpT () =
        let fwdx = Tensor.Create([0.2856; -1.0535; 1.0162; 0.4207; 1.2780]).GetForward(Tensor.Create([-1.9015; 0.4606; -0.1030; 0.0466; -0.2321]))
        let fwdz = fwdx.Exp()
        ...
        let revx = Tensor.Create([0.2856; -1.0535; 1.0162; 0.4207; 1.2780]).GetReverse()
        let revz = revx.Exp()
    ...
    member this.TestDerivativeLogT () =
        let fwdx = Tensor.Create([0.9473; 1.4891; 0.2015; 0.5818; 0.8439]).GetForward(Tensor.Create([1.7164; 0.2905; 1.4872; 1.2580; 0.5778]))
        let fwdz = fwdx.Log()
        ...
        let revx = Tensor.Create([0.9473; 1.4891; 0.2015; 0.5818; 0.8439]).GetReverse()
        let revz = revx.Log()
    ...
    member this.TestDerivativeSqrtT () =
        let fwdx = Tensor.Create([54.7919; 70.6440; 16.0868; 74.5486; 82.9318]).GetForward(Tensor.Create([8.8405; 2.7188; 1.5814; 8.7951; 0.1119]))
        let fwdz = fwdx.Sqrt()
        ...
        let revx = Tensor.Create([54.7919; 70.6440; 16.0868; 74.5486; 82.9318]).GetReverse()
        let revz = revx.Sqrt()
