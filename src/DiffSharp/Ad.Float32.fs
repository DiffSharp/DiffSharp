...
namespace DiffSharp.AD.Float32
...
and DV =
    ...
    member d.Visualize() =
        ...
        | DVR(_) -> sb.AppendLine(sprintf "DVR: %i" d.Length) |> ignore
        for i = 0 to d.Length - 1 do
            if d'.[i] < D 0.f then sb.Append(" ") |> ignore
...
and DM =
    ...
    member d.Visualize() =
        ...
        for i = 0 to d.Rows - 1 do
            for j = 0 to d.Cols - 1 do
                if d'.[i, j] < D 0.f then sb.Append(" ") |> ignore
                else sb.Append("■") |> ignore
            sb.AppendLine() |> ignore
        sb.ToString()
...
module DV =
    ...
    let inline standardBasis (n:int) (i:int) = DV(Array.init n (fun j -> if i = j then 1.f else 0.f))
