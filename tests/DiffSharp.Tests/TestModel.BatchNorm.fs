...
type TestModelBatchNorm () =
    ...
    [<Test>]
    member _.TestModelBatchNorm1dSaveLoadState () =
        let numFeatures = 4
        let batchSize = 2
        let net = BatchNorm1d(numFeatures)
        ...
        Assert.AreEqual(variance0.shape, variance1.shape)
        Assert.AreEqual(mean0, mean2)
        Assert.AreEqual(variance0, variance2)

    [<Test>]
    member _.TestModelBatchNorm2dSaveLoadState () =
        let numFeatures = 4
        let d = 5
        let batchSize = 2
        let net = BatchNorm2d(numFeatures)
    ...
    [<Test>]
    member _.TestModelBatchNorm3dSaveLoadState () =
        let numFeatures = 4
        let d = 5
        let batchSize = 2
        let net = BatchNorm3d(numFeatures)
