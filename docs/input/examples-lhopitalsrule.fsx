...
let lhopital f g n x = dsharp.diffn n f x / dsharp.diffn n g x
...
let l = lhseq (fun x -> 2. * sin x - sin (2. * x)) (fun x -> x - sin x) (v 0.)
...
let lim2 = findlim (fun x -> 2. * sin x - sin (2. * x)) (fun x -> x - sin x) (v 0.)
...
let lim3 = findlim (fun x -> sin (System.Math.PI * x)) (fun x -> System.Math.PI * x) (v 0.)
...
let lim4 = findlim (fun x -> exp x - 1 - x) (fun x -> x * x) (v 0.)
