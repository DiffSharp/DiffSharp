...
// A layer of neurons
type Layer =
    {mutable W:Mat  // Weight matrix
     mutable b:Vec  // Bias vector
     a:Vec->Vec}     // Activation function
...
let runLayer (x:Vec) (l:Layer) =
    l.W * x + l.b |> l.a

let runNetwork (x:Vec) (n:Network) =
...
(**
    ...
    [lang=cs]
    val net2 : Network = {layers = [|{W = Mat [[0.230677625; 0.1414874814]];
                                      b = Vec [|0.4233988253|];}|];}
...
(**
    ...
    val net3 : Network =
      {layers =
        [|{W = Mat [[-0.04536837132; -0.3447727025]
                   [-0.07626016418; 0.06522091877]
                   [0.2581558948; 0.1597980939]];
           b = Vec [|-0.3051199176; 0.2980325892; 0.4621827649|];};
          {W = Mat [[-0.347911722; 0.2696812725; 0.2704776571]];
           b = Vec [|-0.1477482923|];}|];}
...
let runLayer' (x:Mat) (l:Layer') =
    l.W * x + (Mat.createCols x.Cols l.b) |> l.a

let runNetwork' (x:Mat) (n:Network') =
    Array.fold runLayer' x n.layers
...
let backprop' (n:Network') (eta:float) epochs mbsize loss (x:Mat) (y:Mat) =
    let i = DiffSharp.Util.GlobalTagger.Next
    let mutable b = 0
    ...
    while j < epochs do
        ...
        while b < batches do
            ...
            let L:Scalar = loss (runNetwork' mbX n) mbY
            let adjoints = computeAdjoints L  

            for l in n.layers do
                l.W <- primal (l.W.primal - eta * adjoints.[l.W])
                l.b <- primal (l.b.primal - eta * adjoints.[l.b])
...
let l0 = { W = Mat.init 300 784 (fun _ _ -> -0.075 + 0.15 * rnd.NextDouble())
           b = Vec.zeros 300
           a = tanh}

let l1 = { W = Mat.init 10 300 (fun _ _ -> -0.075 + 0.15 * rnd.NextDouble())
           b = Vec.zeros 10
           a = Mat.mapCols softmax}
...
let crossEntropy (x:Mat) (y:Mat) =
    -(x |> Mat.toCols |> Seq.mapi (fun i v -> 
        (Vec.standardBasis v.Length (int (float y.[0, i]))) * log v) |> Seq.sum) / x.Cols
...
// Predicted classes
let testPredict = testY |> Mat.toCols |> Seq.map Vec.maxIndex |> Seq.toArray
...
(**
    ...
    val testTrue : Mat = Mat [[7.0; 2.0; 1.0; 0.0; 4.0]]
*)

for i = 0 to testY.Cols - 1 do
    printfn "Predicted label: %i" testPredict.[i]
    printfn "Image:\n %s" ((mnistTestX.[*,i] |> Vec.mat 28).Visualize())
