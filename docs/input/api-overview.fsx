...
let a = dsharp.diff f (dsharp.tensor 2.)

// Original value of Sin(Sqrt(x)) at x = 2
let b = f (dsharp.tensor 2.)
...
##### curldivergence : $\color{red}{(\mathbb{R}^3 \to \mathbb{R}^3) \to \mathbb{R}^3} \to \color{blue}{(\mathbb{R}^3 \times \mathbb{R})}$
*)
...
(** *)
let v1 = vec [1.; 2.; 3.] // Create DV from sequence of floats
let v2 = Vec.zeros 3   // Create DV of length 3, elements zero
let v3 = Vec.full 3 1.    // Create DV of length 3, each element with value 1.
let v4 = Vec.init 3 f       // Create DV of length 3, compute elements by function f:int->float

(**
#### Basic Operations
*)

let v5 = v1 + v2  // Vector addition
let v6 = v1 - v2  // Vector subtraction
let v7 = v1 * v2  // Vector inner (dot, scalar) product
//let v = v1 &* v2 // Vector outer (dyadic, tensor) product
let v8 = v1 * v2 // Element-wise (Hadamard) product
...
let v17 = 2. / v1  // Divide scalar by each element of vector
...
let v35 = ceil v1
let v36 = round v1
let v37 = v1.softmax(0)
let v38 = v1.softplus()
//let v39 = softsign v1
let v40 = v1.logsumexp(0)
let v41 = v1.sigmoid()
let v42 = v1.relu()
...
//let s1 = Vec.l1norm v1   // L1 norm
//let s2 = Vec.l2norm v1   // L2 norm
//let s3 = Vec.l2normSq v1 // Squared L2 norm

(**
#### Accessing Elements & Conversions
*)

let s4 = v1.[0]    // Element 0
let v43 = v1.[..1]  // Slice of v1, until element 1
let v44 = v1.[1..2] // Slice of v2, between elements 1 and 2
let v45 = vec [1.; 2.; 3.]     // Create DV from sequence of floats
let v46 = vec [1.; 2.; 3.]       // Create DV from sequence of floats
let v47 = vec [v 1.; v 2.; v 3.] // Create DV from sequence of Ds
...
let ss = dsharp.split [2;2;2] (vec [1.; 2.; 3.; 4.; 5.; 6.]) // Split DV into 3 vectors of given lengths
let v45  = dsharp.cat ss // Concatenate sequence of DVs into one
...
let s = Vec.length v1 // Length of DV
let s = Vec.min v1    // Minimum element of DV
let s = Vec.max v1    // Maximum element of DV
let s = Vec.sum v1    // Sum of elements of DV
//let v = Vec.unitDV v1     // Unit vector codirectional with v1
//let v = Vec.normalize v1  // Normalize elements to have zero mean and unit variance

(**
### Matrix Operations

See above for matrix utility code.

#### Creating Matrices
*)
(*** hide ***)
let f1 (i:int) (j:int) = 1.0
let f2 (i:int) = vec [1.0]

(** *)
let m1 = mat [[1.; 2.]; [3.; 4.]] // Create DM from sequence of sequences of floats
let m2 = Mat.zeros 2 2         // Create DM of size 2x2, each element zero
let m3 = Mat.full 2 2 1.          // Create DM of size 2x2, each element with value 1.
let m4 = Mat.init 2 2 f1             // Create DM of size 2x2, compute elements by function f:int->int->float
let m5 = Mat.initRows 2 f2           // Create DM of 2 rows, init rows by function f:int->DV
let m6 = Mat.initCols 2 f2           // Create DM of 2 rows, init columns by function f:int->DV

(**
#### Basic Operations
*)

let m7 = m1 + m2  // Matrix addition
let m8 = m1 - m2  // Matrix subtraction
let m10 = m1 * m2 // Element-wise (Hadamard) product
let m11  = m1 / m2 // Element-wise division
let m12  = m1 ** m2 // Element-wise exponentiation
//let m13 = atan2 m1 m2 // Element-wise atan2

let m = m1.matmul(v) // Matrix-vector product
...
//let s = Mat.det m1              // Determinant
//let m = Mat.inverse m1          // Inverse
//let m = Mat.transpose m1        // Transpose
//let s = Mat.trace m1            // Trace
//let v = Mat.diagonal m1         // Diagonal
//let x = Mat.solve m1 v          // Solve system of linear equations
//let x = Mat.solveSymmetric m1 v // Solve system of linear equations (symmetric)

(**
#### Accessing Elements & Conversions
*)

let s1  = m1.[0, 0]      // Element at 0, 0
let m  = m1.[0..1, 1..] // Slice, between rows 0 and 1, columns 1 and beyond
let m  = m1.[*, 0..1]   // Slice, all rows, between columns 0 and 1
let v1  = m1.[0, *]      // Slice, row 0 as a DV
let v2  = m1.[*, 1]      // Slice, column 1 as a DV
//let vs = Mat.toRows m1   // Return all rows as a sequence of DVs
//let m  = Mat.ofRows [toDV [1.; 2.]; toDV [3.; 4.]] // Create matrix from row vectors
//let vs = Mat.toCols m1   // Return all columns as a sequence of DVs
//let m  = Mat.ofCols [toDV [1.; 3.]; toDV [2.; 4.]] // Create matrix from column vectors
let m  = mat [|[|1.; 2.|]; [|3.; 4.|]|] // Create DM from sequence of sequences of floats
let m  = mat [[1.; 2.]; [3.; 4.]]       // Create DM from sequence of sequences of floats
let m  = mat [[v 1.; v 2.]; [v 3.; v 4.]] // Create DM from sequence of sequences of Ds
//let v  = Mat.toDV m1        // Convert DM to DV by stacking rows of matrix
//let m  = Mat.ofDV 2 v2      // Convert DV to a DM with a given number of rows
//let m  = Mat.appendRow v m  // Append row to matrix
//let m  = Mat.prependRow v m // Prepend row to matrix
//let m  = Mat.appendCol v m  // Append column to matrix
//let m  = Mat.prependCol v m // Prepend column to matrix
...
//let s = Mat.rows m1 // Number of rows
//let s = Mat.cols m1 // Number of columns
//let s = Mat.length m1 // Total number of elements
//let s = Mat.min m1 // Minimum element of DM
//let s = Mat.max m1 // Maximum element of DM
//let s = Mat.sum m1 // Sum of elements of DM
//let m = Mat.normalize // Normalize elements to have zero mean and unit variance
//let m = Mat.mapRows f m1 // Map function f:DV->DV to rows of matrix
//let m = Mat.mapCols f m1 // Map function f:DV->DV to columns of matrix
...
// Create a vector
let v = Vec.init 40 (fun i -> sin ((float i) /2.))

// Create a matrix
let m = Mat.init 20 20 (fun i j -> 1. / exp (((float i) ** 2. + (float j) ** 2.) / 400.))

// Visualize and print to console
//v |> Vec.visualize |> printfn "%s"
//m |> Mat.visualize |> printfn "%s"
