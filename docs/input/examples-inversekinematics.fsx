...
let inverseK (target:Vec) (eta:float) (timeout:int) =
    seq {for i in 0 .. timeout do
            let pos, j = jacobian' forwardK a
            let error = target - pos
            let da = (Mat.inverse j) * error
            a <- a + eta * da
            yield (Vec.norm error, a)
            }
    |> Seq.takeWhile (fun x -> fst x > v 0.4)
    |> Seq.map snd
...
let movement1 = inverseK (vec [5.0; 0.0]) 0.025 100000
let movement2 = inverseK (vec [3.5; -4.0]) 0.025 100000
let movement3 = inverseK (vec [4.5; 4.5]) 0.025 100000
...
let armPoints (a:Vec) =
    let t : float[] = a |> transform |> dsharp.primal |> values
    seq [0., 0.; t.[0], t.[1]; t.[2], t.[3]]

let drawArmLive (aa:seq<Vec>) =
