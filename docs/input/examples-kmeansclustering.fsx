...
let kmeans k eta epsilon (data:Vec[]) =
    // (index of, squared distance to) the nearest mean to x
    let inline nearestm (x:Vec) (means:seq<Vec>) =
        means |> Seq.mapi (fun i m -> i, Vec.normSq (x - m)) |> Seq.minBy snd
    // Squared distance of x to the nearest of the means encoded in w
    let inline dist (w:Vec) (x:Vec) = w |> Vec.splitEqual k |> nearestm x |> snd
    let w0 = Seq.init k (fun _ -> data.[rnd.Next(data.Length)]) |> dsharp.cat
    let wopt = Array.zip data (Array.create data.Length (vec [0.])) |> sgd dist w0 (v eta) (v epsilon)
    let means = Vec.splitEqual k wopt
...
let data = Array.init 200 (fun _ -> (Vec.init 2 (fun _ -> rnd.NextDouble())))
...
//let plotClusters (c:Vec[][]) =
//    List.init c.Length (fun i -> 
//        Chart.Point(Array.map (fun (d:Vec) -> float d.[0], float d.[1]) c.[i], MarkerSize = 10))
//    |> Chart.Combine
...
let data2 = Array.init 10000 (fun _ -> (Vec.init 2 (fun _ -> rnd.NextDouble())))
