...
let helmholtz R (T: float) (b:Vec) (A:Mat) (x:Vec) =
    let bx = b * x
    let oneminbx = 1. - bx
    R * T * Vec.sum(x * log (x / oneminbx))
    - (((* TODO MATMUL *) x * A * x) / (bx * sqrt 8.)) 
...
let testHelmholtz n =
    let R = 1.
    let T = 1.
    let b = Vec.init n (fun _ -> 0.1 * rnd.NextDouble())
    let A = Mat.init n n (fun _ _ -> 0.1 * rnd.NextDouble())
    let x = Vec.init n (fun _ -> 0.1 * rnd.NextDouble())
    helmholtz R T b A x
...
let testHelmholtzGrad n =
    ...
    let A = Mat.init n n (fun _ _ -> 0.1 * rnd.NextDouble())
    let x = Vec.init n (fun _ -> 0.1 * rnd.NextDouble())
    dsharp.grad (helmholtz R T b A) x
