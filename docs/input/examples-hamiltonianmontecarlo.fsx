...
let leapFrog (u:Vec->Scalar) (k:Vec->Scalar) (d:Scalar) steps (x0, p0) =
    let hd = d / 2.
    [1..steps] 
    |> List.fold (fun (x, p) _ ->
        let p' = p - hd * dsharp.grad u x
        let x' = x + d * dsharp.grad k p'
        x', p' - hd * dsharp.grad u x') (x0, p0)
...
let hmc n hdelta hsteps (x0:Vec) (f:Vec->Scalar) =
    let u x = -log (f x) // potential energy
    let k p = (p * p) / v 2. // kinetic energy
    let hamilton x p = u x + k p
    let x = ref x0
    [|for i in 1..n do
        let p = Vec.init (Vec.length x0) (fun _ -> rndn())
...
// x: variable vector
let multiNormal mu sigma (x:Vec) =
    let s = sigma |> Mat.inverse
    exp (-((x - mu) * s * (x - mu)) / v 2.)
...
let samples = 
    multiNormal (vec [0.; 0.]) (mat [[1.; 0.8]; [0.8; 1.]])
    |> hmc 10000 (v 0.1) 10 (vec [0.; 0.])
